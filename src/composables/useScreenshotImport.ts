...
export function useScreenshotImport() {
  ...
  async function processImage(file: File | Blob): Promise<void> {
    ...
    try {
      ...
      const cells = await detectGridFromFile(file)
      ...
      for (let i = 0; i < nonEmpty.length; i++) {
        ...
        const queryHash = computeDHash(cell.imageData)
        ...
        // 3b: Match against sprite hash database via Hamming distance
        ...
        if (match.ambiguous && match.groupId) {
          // Resolve the duplicate group member names from the database.
          // duplicateGroupMap: groupId → string[] of item names.
